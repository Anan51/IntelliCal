...
export default function SyllabusIntake({ embedded = false, onCommitted }: Props) {
  ...
  const body = (
    <>
      ...
      {parseDrafts.length === 0 ? (
        <Empty>Parsed events appear here for review before anything hits your calendar.</Empty>
      ) : (
        <div className="review-table-wrap">
          <div className="review-toolbar">
            <Badge tone="accent">{parseDrafts.length} draft events</Badge>
            <Button onClick={handleCommit}>Commit to calendar</Button>
  ...
  return (
    <Section
      ...
      subtitle="Paste a syllabus, review extracted events, then commit."
      actions={null}
    >
      {body}
    </Section>
